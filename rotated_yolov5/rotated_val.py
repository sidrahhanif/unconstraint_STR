...
from rbox_iou_cal import box_intersection_area, vertices2area
...
def rbox_iou(box1, box2):
    ...
    for i in range(len(box1)):
        ...
        for j in range(len(box2)):
            box1_i = np.array([[box1[i][0], box1[i][1]], [box1[i][2], box1[i][3]], [box1[i][4], box1[i][5]], [box1[i][6], box1[i][7]]])
            box2_j = np.array([[box2[j][0], box2[j][1]], [box2[j][2], box2[j][3]], [box2[j][4], box2[j][5]], [box2[j][6], box2[j][7]]])

            intersect = box_intersection_area(box1_i, box2_j)
            if intersect !=0:
                intersect = intersect[0]
            area1, vertices1 = vertices2area(box1_i)
            area2, vertices2 = vertices2area(box2_j)

            if intersect>= area1 or intersect>= area2:
                intersect = min(area1,area2)


            iou_box1_box2 = intersect/(area1 + area2 - intersect)
            if iou_box1_box2 < 0 or iou_box1_box2 > 1 :
                print(box1_i, box2_j)
                print()
            temp.append(iou_box1_box2)
        result.append(temp)
...
def process_batch(detections, labels, iouv):
    ...
    iou = rbox_iou(labels[:, 1:], detections[:, :8]).to(iouv.device)
...
def run(data,
    ...
    for batch_i, (im, targets, paths, shapes) in enumerate(pbar):
        ...
        for si, pred in enumerate(out): # pred (tensor): (n, [xylsθ, conf, cls])
            ...
            pred_hbb = torch.cat((poly, pred_poly[:, -2:]), dim=1) # (n, [xyxyxyxy, conf, cls])
            ...
            pred_hbbn = torch.cat((pred_polyn[:, :8], pred_polyn[:, -2:]), dim=1) # (n, [xyxy, conf, cls]) native-space pred
            ...
            if nl:
                ...
                labels_hbbn = torch.cat((labels[:, 0:1], tpoly), 1)  # native-space labels (n, [cls xyxyxyxy])
