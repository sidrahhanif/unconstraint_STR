...
def save_one_rotated_box(poly, im, file='image.jpg', gain=1.02, pad=10, square=False, BGR=False, save=True):
    """
    # Save image crop as {file} with crop size multiple {gain} and {pad} pixels. Save and/or return crop
    xyxy = torch.tensor(xyxy).view(-1, 4)
    b = xyxy2xywh(xyxy)  # boxes
    ...
    center , (width, height), theta = cv2.minAreaRect(poly)
    shape = (im.shape[1], im.shape[0])  # cv2.warpAffine expects shape in (length, height)
    ...
    if crop.shape[0] > crop.shape[1]*1.8 :
        ...
        for index in range(len(crop)):
            crop[index] = crop[index][::-1]
    """
    if save:
        file.parent.mkdir(parents=True, exist_ok=True)  # make directory
        filepath = str(increment_path(file).with_suffix('.jpg'))
        cv2.imwrite(filepath, crop)
    return center[0], center[1], width, height, theta,filepath
