...
from utils.plots import Annotator, colors, save_one_box, save_one_rotated_box
            ...
            if len(det):
                ...
                for *poly, conf, cls in reversed(det):
                    ...
                    if save_crop: # Yolov5-obb doesn't support it yet
                        ...
                        x_b,y_b,w_b,h_b,theta_back, filepath = save_one_rotated_box(poly, imc, file=save_dir / 'crops' / names[c] / f'{p.stem}.jpg', BGR=True)

                    if save_txt:  # Write to file
                        # xywh = (xyxy2xywh(torch.tensor(xyxy).view(1, 4)) / gn).view(-1).tolist()  # normalized xywh

                        line = (cls, *poly, conf) if save_conf else (cls, x_b,y_b,w_b,h_b, theta_back) # label format
                        with open(txt_path + '.txt', 'a') as f:
                            f.write(filepath +' ' +('%g ' * len(line)).rstrip() % line + '\n')
...
def parse_opt():
    ...
    parser.add_argument('--source', type=str, default='dataset/hand_writing_images/images2/', help='file/dir/URL/glob, 0 for webcam')
