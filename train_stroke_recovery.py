...
def graph(batch,
    ...
    def subgraph(coords, gt_img, name, is_gt=True):
        ...
        # Red images
        gt_np = gt_img.numpy()
        bg = overlay_images(background_img=gt_np, foreground_gt=coords.transpose(), linewidth=2)
        ...
        reconstruction = overlay_images(background_img=np.ones(gt_np.shape),
                                        foreground_gt=coords.transpose(),name=name,  color=[0,0,0],
                                        linewidth=2)
        ...
        #coords[2:] = 0
        plot_points = True #False # True #False #False
        if plot_points:
