...
#eval_img_path_override = PROJ_ROOT / Path("/home/tug85766/Trace/data_processing/online_coordinate_data/MAX_stroke_vNORMAL_TRAINING_TESTFull_complete_img/images/")
#eval_img_path_override = PROJ_ROOT / Path("./word_detection/runs/detect/exp12/crops/word/")
eval_img_path_override = Path('/dev/shm/strokes_recovery/IAM_example_words/')
                              #/dev/shm/strokes_recovery/Signed_data_writing/sample_images_deliver_results/crops/')
#Path('/home/tug85766/Word_detection_STR_product/word_detection/runs/detect/exp18/crops/word/')
#Path('/home/tug85766/Word_detection_STR_product/word_detection/runs/detect/exp16/crops/word/')
    #'Path('/home/tug85766/MAX_stroke_vNORMAL_TRAINING_TEST_testing_full_page_with_cv2_10927_no_lr_padding/test_images/')
#Path('/home/tug85766/MAX_stroke_vNORMAL_TRAINING_TEST_testing_full_page_with_cv2_10927_no_lr_padding/test_images/')
#Path("/home/tug85766/Word_detection_STR_product/word_detection/runs/detect/exp14/crops/word/") #
                           #"/dev/shm/strokes_recovery/MAX_stroke_vNORMAL_TRAINING_TEST_testing_full_page_same_testing_width/images/")
...
def main(config_path):
    ...
    #OUTPUT = PROJ_ROOT / Path("./example_weights/results/")
    ...
    model = StrokeRecoveryModel(**config.model_definition)
    device = torch.device("cuda:2" if torch.cuda.is_available() else "cpu")
    # Send the model to GPU
    #model_ft = nn.DataParallel(model, device_ids=[3,4])
...
def eval_only(dataloader, model):
    ...
    final_out = []
    kd_trees = {}
    distance_gt_to_nearest_pred, distance_pred_to_nearest_gt = [], []

    for i, item in enumerate(tqdm(dataloader)):
        preds = TrainerStrokeRecovery.eval(item["line_imgs"],
                                           model,
                                           label_lengths=item["label_lengths"],
                                           relative_indices=config.pred_relativefy, device=device,
        ...
        for ii, p in enumerate(preds): # Loop within batch
            ...
            post_process(p, gt, calculate_distance=True, kd=kd)
            distance = np.average(distances)
            # MOVE GT TO MATCH PRED - too expensive
            if item_number < 200: # 3
                _, distances2, _ = stroke_recovery.get_nearest_point(pred, gt, reference_is_image=False)
    ...
    #utils.pickle_it(final_out, output_path / f"all_data.pickle")
        distance_gt_to_nearest_pred.append(avg_distance)
        distance_pred_to_nearest_gt.append(avg_distance2)
...
def load_all_gts(gt_path):
    ...
    #np.save(r"/home/tug85766/Trace/data_processing/prepare_IAM_Lines/RESULTS/OFFLINE_PREDS/TEXT.npy", GT_DATA)
    return GT_DATA
