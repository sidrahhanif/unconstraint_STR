...
class CRAFT_yolov5_v4(nn.Module):
    def __init__(self,opt, pretrained=True, freeze=False):
        super(CRAFT_yolov5_v4, self).__init__()

        craft_model = CRAFT()
        craft_model.load_state_dict(copyStateDict(torch.load('/raid/tmp/Text_detection/CRAFT/data/real_weights_gnhk/CRAFT_clr_99.pth', map_location='cpu')))
        #craft_model = craft_model.to(opt.device)
        ...
        if pretrained:
            ...
            model_yolo = Model(opt.cfg, ch=3, nc=1, anchors = 3) #=opt.hyp.get('anchors')).to(opt.device) #,  # create #
    ...
    def forward(self, x, char_mode):
        ...
        if char_mode == False:
            #import numpy as np
            #o = np.squeeze((y[:, :, :, 0] * 255).detach().cpu().numpy())
            regionscores_yolo = y[:, :, :, 0]* 255  # uint8 to float32, 0-255 to 0.0-1.0
            ...
            y_yolo = self.model_yolo(regionscores_yolo)
            return y, y_yolo, feature
        else:
            return y,  feature
